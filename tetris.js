...
function draw() {
    ...
    // 画面クリアと背景描画
    ctx.clearRect(0, 0, 32 * size, 24 * size);
    ctx.fillStyle = "#0e101f";
    ctx.fillRect(0, 0, 32 * size, 24 * size);
...
function line_delete() {
    ...
    if (t_spined) {
        console.log("T-Spin " + ["", "Single", "Double", "Triple"][deleted_lines]);
    }else{
        if(deleted_lines>0){
            console.log(["", "Single", "Double", "Triple", "Tetris"][deleted_lines]);
    }}
    // パーフェクトクリア
